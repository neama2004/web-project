document.addEventListener("DOMContentLoaded", function() {
  ...
  function showAllergyManagementModal(user) {
    ...
    const allergyData = [
      { name: 'Gluten', icon: 'fa-bread-slice', description: 'Wheat, barley, rye' },
      { name: 'Dairy', icon: 'fa-cheese', description: 'Milk, cheese, yogurt' },
      { name: 'Nuts', icon: 'fa-acorn', description: 'Almonds, walnuts, cashews' },
      { name: 'Peanuts', icon: 'fa-peanut', description: 'Peanuts and peanut products' },
      { name: 'Soy', icon: 'fa-seedling', description: 'Soybeans and soy products' },
      { name: 'Eggs', icon: 'fa-egg', description: 'Chicken eggs and derivatives' },
      { name: 'Fish', icon: 'fa-fish', description: 'All fish varieties' },
      { name: 'Shellfish', icon: 'fa-shrimp', description: 'Shrimp, crab, lobster' }
    ];
  
    content.innerHTML = `
      <div class="allergy-modal-header">
        <h2><i class="fas fa-allergies"></i> Manage Allergy Profile</h2>
        <p>Select all that apply to you. We'll use this to filter recommendations.</p>
      </div>
      <form id="allergy-form">
        <div class="checkbox-group">
          ${allergyData.map(allergy => `
            <label class="allergy-option ${allergy.name.toLowerCase()}">
              <input type="checkbox" name="${allergy.name.toLowerCase()}" 
                    ${(user.allergies || []).includes(allergy.name) ? "checked" : ""}>
              <div class="allergy-card">
                <i class="fas ${allergy.icon} allergy-icon"></i>
                <span class="allergy-name">${allergy.name}</span>
                <small class="allergy-desc">${allergy.description}</small>
              </div>
            </label>
          `).join("")}
        </div>
        <div class="form-actions">
          ...
          <button type="submit" class="save-btn">Save Allergy Profile</button>
        </div>
      </form>`;
