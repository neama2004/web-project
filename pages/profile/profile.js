document.addEventListener('DOMContentLoaded', function() {
    ...
    function populateUserData() {
      document.getElementById('profile-name').textContent = currentUser.name || "Guest User";
      document.getElementById('user-fullname').textContent = currentUser.name || "Guest User";
      document.getElementById('user-email').textContent = currentUser.email || "Not provided";
      ...
      document.getElementById('allergies-count').textContent = currentUser.allergies.length;
            ...
            <div class="order-items">
              ${order.items.slice(0, 2).map(item => `
                <div class="order-item">
                  <img src="../../images/products/${item.image}" alt="${item.name}" onerror="this.src='../../images/products/default-product.jpg'">
    ...
    document.addEventListener('click', function(e) {
      if (e.target.classList.contains('order-action-btn')) {
        ...
        const action = button.classList.contains('reorder') ? 'reorder' : 
                       button.classList.contains('track') ? 'track' : 'details';
    ...
    document.getElementById('view-all-orders').addEventListener('click', function(e) {
      e.preventDefault();
    ...
    function trackOrder(order) {
      ...
      const statusMessages = {
        ...
        'shipped': `Shipped on ${new Date(order.shippingDate).toLocaleDateString()}`,
        'delivered': `Delivered on ${new Date(order.deliveryDate).toLocaleDateString()}`,
        'cancelled': 'This order has been cancelled'
      };
      
      showNotification(`Order #${order.id}: ${statusMessages[order.status] || 'Status unknown'}`);
    }
    ...
    function showOrderDetails(order) {
      ...
      const detailsHtml = `
        ...
        <div class="order-detail-info">
          ...
          ${order.paymentMethod ? `<p><strong>Payment Method:</strong> ${order.paymentMethod.type} ending in ${order.paymentMethod.last4}</p>` : ''}
        </div>
        ...
        <div class="order-detail-actions">
          <button class="order-action-btn reorder" data-order-id="${order.id}">Reorder All Items</button>
        </div>
      `;
    ...
    const modalContent = `
        ...
        <form id="allergy-form">
            <div class="checkbox-group">
                <label>
                    <input type="checkbox" name="gluten" ${currentUser.allergies.includes('gluten') ? 'checked' : ''}>
                    Gluten
                </label>
                <label>
                    <input type="checkbox" name="dairy" ${currentUser.allergies.includes('dairy') ? 'checked' : ''}>
                    Dairy
                </label>
                <label>
                    <input type="checkbox" name="nuts" ${currentUser.allergies.includes('nuts') ? 'checked' : ''}>
                    Tree Nuts
                </label>
                <label>
                    <input type="checkbox" name="peanuts" ${currentUser.allergies.includes('peanuts') ? 'checked' : ''}>
                    Peanuts
                </label>
                <label>
                    <input type="checkbox" name="soy" ${currentUser.allergies.includes('soy') ? 'checked' : ''}>
                    Soy
                </label>
                <label>
                    <input type="checkbox" name="eggs" ${currentUser.allergies.includes('eggs') ? 'checked' : ''}>
                    Eggs
                </label>
                <label>
                    <input type="checkbox" name="fish" ${currentUser.allergies.includes('fish') ? 'checked' : ''}>
                    Fish
                </label>
                <label>
                    <input type="checkbox" name="shellfish" ${currentUser.allergies.includes('shellfish') ? 'checked' : ''}>
                    Shellfish
                </label>
            </div>
